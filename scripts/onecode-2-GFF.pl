...
for my $file (glob '*.elem_sorted.csv') {
    ...
    while (<$FH>) {
    	...
    	if ( 
    		...
    		($line[10] =~ /LTR\/*.*/) &&			# is the Order fiels, such as LTR, LINE, CR1, etc. 
    	...
    	print OUT "$line[4]\tOneCode\trepeat_region\t$line[5]\t$line[6]\t$line[1]\t$strand\t.\tFamily \"$element\"; Superfamily \"$superfam\"; Order \"$order\"; Perc_ref \"$line[16]\";\n";  	 
...
sub print_usage {
  ...
  print "IMPORTANT:    $0 must be called from the same directory where the output of Onecode is, as it will look for *.elem_sorted.csv files ... hence, there is no input declaration in the command call\n\n";
  print "INPUT:        <perc_ref>            float - for each TE feature, Onecode reports the percentage length of the reference. This value sets the minimim. For strict parameter, use 0.8\n";
