...
sub Wanted_length{
  ...
  if(/.*\.out$/  | ($file_ok==1)){
    ...
    while(<FILE>){
      ...
      next unless(($F[10] =~ /^DNA/) | ($F[10] =~/^SINE/) | ($F[10] =~ /^LINE/) | ($F[10] =~ /^LTR/) | ($F[10] =~ /^PLE/) | (($F[10] =~/^Unknown|^Unspecified/) & ($unknown == 1)));
...
sub Wanted{
  if(/.*\.out$/ | ($file_ok==1)){
	...
	next;
      ...
      next unless(($F[10] =~ /^DNA/) | ($F[10] =~/^SINE/) | ($F[10] =~ /^LINE/) | ($F[10] =~ /^LTR/) | ($F[10] =~ /^PLE/) | ($F[10] =~ /^RC/) | (($F[10] =~/^Unknown|^Unspecified/) & ($unknown == 1)));
	...
	foreach $elem (sort keys %{$h{$contig}{$family}}){
      ...
      @families=("Type:DNA","Type:LINE","Type:SINE","Type:LTR","Type:PLE");
      if($unknown==1){
	  ...
	  ### Making sums inside each category (class, but also DNA, LINE, SINE, PLE...)
